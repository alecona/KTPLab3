...
public class AStarState
{
    ...
    private HashMap<Location, Waypoint> closeWaypoint = new HashMap<Location, Waypoint>();;
    ...
    public Waypoint getMinOpenWaypoint()
    {
        ...
        if (!openWaypoints.isEmpty()) {
            ...
            for (Waypoint NWpoint : openWaypoints.values()) {
                if (NWpoint.getTotalCost() < minCost) {
                    minCost = NWpoint.getTotalCost();
                    minCostPoint = NWpoint;
                }
            }
        }
        return minCostPoint;
    ...
    public boolean addOpenWaypoint(Waypoint newWP)
    {
        ...
        if (openWaypoints.containsKey(location)) {
            if (newWP.getPreviousCost() < openWaypoints.get(location).getPreviousCost())
            {
                openWaypoints.put(location, openWaypoints.get(location));
                return true;
            }
            return false;
        }
